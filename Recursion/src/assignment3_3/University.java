...
public class University implements UniversityInterface {
	...
	public void add_faculty(String name, String rec_date, float salary, int teaching_load) {
		 ...
		 * the salary of the Employee. teaching_load: int representing the teaching load
		 * of the Employee.
		 */
	...
	public static void main(String[] args) {
		...
		while (true) { // loop while the user is still using the program
			...
			if (choice == 1) {
				...
				while ((!(emp.toLowerCase()).equals("faculty")) && (!(emp.toLowerCase()).equals("staff"))) { // Stuck in
																												// this
																												// loop
																												// while
																												// until
				...
				for (int i = 0; i < University.size; i++) { // Search for the indexes of the employee having a specific
															// name to delete
				...
				if (!indexes.equals("")) {
					for (int i = 0; i < indexes.length(); i++) { // Ask the user if he wants to delete the employee
																	...
																	// be asked if he wants to delete a specific
																	// employee
																	// if this is not the employee in mind ask for the
																	// other
						...
						while (!del.equalsIgnoreCase("y") && !del.equalsIgnoreCase("n")) { // Stuck in this loop while
				...
				if (!indexes.equals("")) {
					...
					for (j = 0; j < indexes.length(); j++) { // Ask the user if he wants to delete the employee
						...
						if (j == indexes.length() - 1)
							...
							System.out.print("For Yes press \"y\", for No press \"n\": ");
							String del = scan.nextLine();
							while (!del.equalsIgnoreCase("y") && !del.equalsIgnoreCase("n")) { // Stuck in this loop
																								// while until the user
																								// enters good input
																								// (salary)
								System.out.print("For Yes press \"y\", for No press \"n\": ");
								del = scan.nextLine();
				...
				while (!exit.equalsIgnoreCase("y") && !exit.equalsIgnoreCase("n")) { // Stuck in this loop while until
																						// the user enters good input
																						// (salary)
					System.out.print("For Yes press \"y\", for No press \"n\": ");
					exit = scan.nextLine();
				}
